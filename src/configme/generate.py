...
def generate_makefile(root: Path, machine: str, compiler: str,
    ...
    header = (
        ...
        f"#\n"
        f"# MACHINE/COMPILER are exposed as make variables so a template can select\n"
        f"# machine/compiler-specific rules (e.g. fesm-utils' config/libs.mk keys its\n"
        f"# FFTW/LIS/SHTns autotools quirks on them). Unused elsewhere — harmless.\n"
        f"MACHINE = {machine}\n"
        f"COMPILER = {compiler}"
    )
